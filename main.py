...
# ফ্লাস্ক অ্যাপ তৈরি (Render পোর্ট এরর ফিক্স করার জন্য)
app = Flask(__name__)
...
def sync_to_firebase(path, data):
    try: requests.put(f"{FB_BASE_URL}/{path}.json", json=data, timeout=5)
    except: pass
...
if __name__ == "__main__":
    # থ্রেডগুলো ব্যাকগ্রাউন্ডে চালু করা
    threading.Thread(target=wingo_task, daemon=True).start()
    threading.Thread(target=d5_task, daemon=True).start()
    threading.Thread(target=k3_task, daemon=True).start()
    
    # Render পোর্ট ফিক্স করার জন্য ফ্লাস্ক সার্ভার রান করা
